...
g0<-ggplot(data=r, aes(x = Var2, y = Var1)) +
      labs(x = "x", y = "y", fill = "density") +
      geom_raster(aes(fill=newvals)) +
      theme_bw(20)+
    ...
    g0<-g0+ geom_point(data=datax,aes(x=x,y=y_height-y,colour=llh))+scale_colour_gradientn(colours=jet.colors(7))
    
    savename=paste('Im', im, '_0.pdf',sep='')
    ggsave(filename = savename,plot = g0, path = 'Heatmaps4',width = 8,height=6,unit='in')
...
#   ggsave(filename = savename,plot = g1, path = 'Heatmaps4',width = 8,height=6,unit='in')
#   
...
#   ggsave(filename = savename,plot = g2, path = 'Heatmaps4',width = 8,height=6,unit='in')
#   
...
#   ggsave(filename = savename,plot = g3, path = 'Heatmaps4',width = 8,height=6,unit='in')
#   
#   
...
#   ggsave(filename = savename,plot = g4, path = 'Heatmaps4',width = 8,height=6,unit='in')
