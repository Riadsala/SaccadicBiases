...
#   
  ...
  cols=c("#00007F", "blue", "#007FFF", "cyan",
    "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000")
  jet.colors2 <-
    colorRampPalette(cols[length(cols):1])
  
  
  
  g0<-ggplot(r, aes(x = Var2, y = Var1)) +
  ...
  g1<-g0+geom_point(data=datax,aes(x=x,y=y_height-y,colour=llh),size=5,alpha=0.5)+
    scale_colour_gradientn(colours=jet.colors2(10))
  
  ##now scale llh by top 20% of fixs
  llhranked<-datax$llh[order(datax$llh)]
  takefrom=llhranked[round(nrow(datax)/100*20)]
  
  g2<-g0+geom_point(data=subset(datax,llh<=takefrom),aes(x=x,y=y_height-y,colour=llh),size=5,alpha=0.5)+
    scale_colour_gradientn(colours=jet.colors2(10))
  ...
  #ggsave(filename=paste('Im',im,'.pdf',sep=''),width=16,height=6,units='in')
